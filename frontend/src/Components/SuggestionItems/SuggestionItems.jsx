...
function SuggestionList(props){
  ...
  return ( 
    ...
    <div className="suggestions" >
      {suggestion?.products?.slice(0,4).map((suggestionsItem) => {
        return (
        <>

        <div key={suggestionsItem._id} style={{ background: '#FBD3D3' }} className="suggested-products">
            <button className={suggestionsItem.isComplete ? "wishlist-click active" : "wishlist"} type='primary' shape='rectangle' key={suggestionsItem._id} onClick={() => wishLists(suggestionsItem._id)}>
